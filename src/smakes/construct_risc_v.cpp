...
bool Ctor_RV::rv_assembly0_to_bin(std::string if_name, std::string of_name) {
    ...
    if(!fin.is_open()) {
        std::cout << SStyle::RED << "Error constructing rv assembly 0 to binary." << SStyle::NC << std::endl;
    ...
    if(!fout.is_open()) {
        std::cout << SStyle::RED << "Error constructing rv assembly 0 to binary." << SStyle::NC << std::endl;
        std::cout << SStyle::RED << "Error opening the output file: " << of_name << "." << SStyle::NC << std::endl;
        return false;
    }
...
bool Ctor_RV::rv_assembly1_to_assembly0(std::string if_name, std::string of_name) {
    ...
    // Check if the input file failed to open
    if(!fin.is_open()) {
        std::cout << SStyle::RED << "Error constructing rv assembly 1 to assembly 0." << SStyle::NC << std::endl;
        std::cout << SStyle::RED << "Error opening the input file: " << if_name << "." << SStyle::NC << std::endl;
        return false;
    }
    // Check if the output file failed to open
                ...
                if(ss_line >> rs1 >> rs2 >> imm) {
                    ...
                    if(ss_line >> rs1 >> rs2 >> symbol) {
                        ...
                        if(itr != symbol_table.end()) {
                            ...
                            fout << "!!! Error line !!!" << std::endl;
                            std::cout << SStyle::RED << "Error constructing rv assembly 1 to rv assembly 0." << SStyle::NC << std::endl;
